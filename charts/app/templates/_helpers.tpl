...
{{ $k }}: {{ $v }}
{{- end }}
{{- if and .Values.global.aws.accountId (or .Values.infra.s3Bucket.enabled .Values.infra.bedrock.enabled .Values.infra.textract.enabled .Values.infra.eventing.producer.enabled .Values.infra.eventing.consumer.enabled (not (empty .Values.infra.dynamodb.tables)) .Values.infra.opensearch.enabled) }}
